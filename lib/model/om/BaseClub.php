...
abstract class BaseClub extends BaseObject  implements Persistent
{
	...
	/**
	 * The value for the nombre_club field.
	 * @var        string
	 */
	protected $nombre_club;
	...
	/**
	 * Get the [nombre_club] column value.
	 * 
	 * @return     string
	 */
	...
	/**
	 * Set the value of [nombre_club] column.
	 * 
	 * @param      string $v new value
	 * @return     Club The current object (for fluent API support)
	 */
	public function setNombreClub($v)
	{
		if ($v !== null) {
			$v = (string) $v;
		}

		if ($this->nombre_club !== $v) {
			$this->nombre_club = $v;
			$this->modifiedColumns[] = ClubPeer::NOMBRE_CLUB;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->nombre_club = ($row[$startcol + 1] !== null) ? (string) $row[$startcol + 1] : null;
			$this->resetModified();
			...
			return $startcol + 2; // 2 = ClubPeer::NUM_HYDRATE_COLUMNS.
	...
	public function delete(PropelPDO $con = null)
	{
		...
		try {
			...
			foreach (sfMixer::getCallables('BaseClub:delete:pre') as $callable)
			{
			...
			if ($ret) {
				...
				$this->postDelete($con);
				// symfony_behaviors behavior
				foreach (sfMixer::getCallables('BaseClub:delete:post') as $callable)
				{
	...
	public function save(PropelPDO $con = null)
	{
		...
		try {
			...
			// symfony_behaviors behavior
			foreach (sfMixer::getCallables('BaseClub:save:pre') as $callable)
			{
			...
			if ($ret) {
				...
				$this->postSave($con);
				// symfony_behaviors behavior
				foreach (sfMixer::getCallables('BaseClub:save:post') as $callable)
				{
	...
	protected function doInsert(PropelPDO $con)
	{
		...
		if ($this->isColumnModified(ClubPeer::NOMBRE_CLUB)) {
			$modifiedColumns[':p' . $index++]  = '`NOMBRE_CLUB`';
		}
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[1], $arr)) $this->setNombreClub($arr[$keys[1]]);
	}
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(ClubPeer::NOMBRE_CLUB)) $criteria->add(ClubPeer::NOMBRE_CLUB, $this->nombre_club);
	...
	{
		$copyObj->setNombreClub($this->getNombreClub());
	...
	public function __call($name, $params)
	{
		...
		// symfony_behaviors behavior
		if ($callable = sfMixer::getCallable('BaseClub:' . $name))
		{
