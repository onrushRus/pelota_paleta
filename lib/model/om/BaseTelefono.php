...
abstract class BaseTelefono extends BaseObject  implements Persistent
{
	...
	/**
	 * The value for the telefono_nro field.
	 * Note: this column has a database default value of: 'S/N'
	 * @var        string
	 */
	protected $telefono_nro;
	...
	/**
	 * Get the [telefono_nro] column value.
	 * 
	 * @return     string
	 */
	public function getTelefonoNro()
	{
		return $this->telefono_nro;
	}
	...
	public function setPersonaNroDoc($v)
	{
		...
		if ($this->persona_nro_doc !== $v) {
			...
			$this->modifiedColumns[] = TelefonoPeer::PERSONA_NRO_DOC;
		}
	...
	/**
	 * Set the value of [telefono_nro] column.
	 * 
	 * @param      string $v new value
	 * @return     Telefono The current object (for fluent API support)
	 */
	public function setTelefonoNro($v)
	{
		if ($v !== null) {
			$v = (string) $v;
		}

		if ($this->telefono_nro !== $v) {
			$this->telefono_nro = $v;
			$this->modifiedColumns[] = TelefonoPeer::TELEFONO_NRO;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->telefono_nro = ($row[$startcol + 1] !== null) ? (string) $row[$startcol + 1] : null;
			$this->resetModified();
			...
			return $startcol + 2; // 2 = TelefonoPeer::NUM_HYDRATE_COLUMNS.
	...
	protected function doInsert(PropelPDO $con)
	{
		...
		if ($this->isColumnModified(TelefonoPeer::PERSONA_NRO_DOC)) {
			$modifiedColumns[':p' . $index++]  = '`PERSONA_NRO_DOC`';
		}
		if ($this->isColumnModified(TelefonoPeer::TELEFONO_NRO)) {
			$modifiedColumns[':p' . $index++]  = '`TELEFONO_NRO`';
		}
	...
	{
		if (isset($alreadyDumpedObjects['Telefono'][serialize($this->getPrimaryKey())])) {
			return '*RECURSION*';
		}
		$alreadyDumpedObjects['Telefono'][serialize($this->getPrimaryKey())] = true;
		$keys = TelefonoPeer::getFieldNames($keyType);
		$result = array(
			$keys[0] => $this->getPersonaNroDoc(),
			$keys[1] => $this->getTelefonoNro(),
		);
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[1], $arr)) $this->setTelefonoNro($arr[$keys[1]]);
	}
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(TelefonoPeer::PERSONA_NRO_DOC)) $criteria->add(TelefonoPeer::PERSONA_NRO_DOC, $this->persona_nro_doc);
		if ($this->isColumnModified(TelefonoPeer::TELEFONO_NRO)) $criteria->add(TelefonoPeer::TELEFONO_NRO, $this->telefono_nro);
	...
	{
		$criteria = new Criteria(TelefonoPeer::DATABASE_NAME);
		$criteria->add(TelefonoPeer::PERSONA_NRO_DOC, $this->persona_nro_doc);
		$criteria->add(TelefonoPeer::TELEFONO_NRO, $this->telefono_nro);
	...
	{
		return (null === $this->getPersonaNroDoc()) && (null === $this->getTelefonoNro());
	}
	...
	public function copyInto($copyObj, $deepCopy = false, $makeNew = true)
	{
		...
		$copyObj->setTelefonoNro($this->getTelefonoNro());
	...
	public function getPersona(PropelPDO $con = null)
	{
		if ($this->aPersona === null && ($this->persona_nro_doc !== null)) {
			...
			/* The following can be used additionally to
				...
				$this->aPersona->addTelefonos($this);
			 */
		}
