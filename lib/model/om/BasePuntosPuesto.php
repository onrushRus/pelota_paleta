...
/**
 * Base class that represents a row from the 'puntos_puesto' table.
 *
 * 
 *
 * @package    propel.generator.lib.model.om
 */
abstract class BasePuntosPuesto extends BaseObject  implements Persistent
{

	/**
	 * Peer class name
	 */
	const PEER = 'PuntosPuestoPeer';

	/**
	 * The Peer class.
	 * Instance provides a convenient way of calling static methods on a class
	 * that calling code may not be able to identify.
	 * @var        PuntosPuestoPeer
	 */
	 ...
	 * @return     int
	 */
	public function getPuntosPorPuesto()
	{
	...
	public function setId($v)
	{
		...
		if ($this->id !== $v) {
			$this->id = $v;
			$this->modifiedColumns[] = PuntosPuestoPeer::ID;
		}
	 ...
	 * @return     PuntosPuesto The current object (for fluent API support)
	 */
	public function setPuntosPorPuesto($v)
	{
		if ($v !== null) {
			$v = (int) $v;
		}

		if ($this->puntos_por_puesto !== $v) {
			$this->puntos_por_puesto = $v;
			$this->modifiedColumns[] = PuntosPuestoPeer::PUNTOS_POR_PUESTO;
		}
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		$stmt = PuntosPuestoPeer::doSelectStmt($this->buildPkeyCriteria(), $con);
	...
	public function delete(PropelPDO $con = null)
	{
		...
		try {
			$deleteQuery = PuntosPuestoQuery::create()
				->filterByPrimaryKey($this->getPrimaryKey());
			$ret = $this->preDelete($con);
			// symfony_behaviors behavior
			foreach (sfMixer::getCallables('BasePuntosPuesto:delete:pre') as $callable)
			{
			...
			if ($ret) {
				...
				$this->postDelete($con);
				// symfony_behaviors behavior
				foreach (sfMixer::getCallables('BasePuntosPuesto:delete:post') as $callable)
				{
	...
	public function save(PropelPDO $con = null)
	{
		...
		try {
			...
			// symfony_behaviors behavior
			foreach (sfMixer::getCallables('BasePuntosPuesto:save:pre') as $callable)
			{
			...
			if ($ret) {
				...
				$this->postSave($con);
				// symfony_behaviors behavior
				foreach (sfMixer::getCallables('BasePuntosPuesto:save:post') as $callable)
				{
				  call_user_func($callable, $this, $con, $affectedRows);
				}

				PuntosPuestoPeer::addInstanceToPool($this);
			} else {
				$affectedRows = 0;
			}
			$con->commit();
	...
	protected function doInsert(PropelPDO $con)
	{
		...
		$sql = sprintf(
			'INSERT INTO `puntos_puesto` (%s) VALUES (%s)',
	...
	protected function doValidate($columns = null)
	{
		if (!$this->alreadyInValidation) {
			...
			if (($retval = PuntosPuestoPeer::doValidate($this, $columns)) !== true) {
	...
	{
		$pos = PuntosPuestoPeer::translateFieldName($name, $type, BasePeer::TYPE_NUM);
	...
	{
		if (isset($alreadyDumpedObjects['PuntosPuesto'][$this->getPrimaryKey()])) {
			return '*RECURSION*';
		}
		$alreadyDumpedObjects['PuntosPuesto'][$this->getPrimaryKey()] = true;
	...
	{
		$criteria = new Criteria(PuntosPuestoPeer::DATABASE_NAME);
		$criteria->add(PuntosPuestoPeer::ID, $this->id);
	 ...
	 * objects.
	 *
	 * @param      object $copyObj An object of PuntosPuesto (or compatible) type.
	 ...
	 *
	 * @param      boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
	 * @return     PuntosPuesto Clone of current object.
	 * @throws     PropelException
	 */
	 ...
	 *
	 * @return     PuntosPuestoPeer
	 */
	public function getPeer()
	{
		if (self::$peer === null) {
			self::$peer = new PuntosPuestoPeer();
		}
		return self::$peer;
	}
	 ...
	 * If this PuntosPuesto is new, it will return
	...
	public function setResultadoTorneos(PropelCollection $resultadoTorneos, PropelPDO $con = null)
	{
		...
		foreach ($resultadoTorneos as $resultadoTorneo) {
			...
			if ($resultadoTorneo->isNew()) {
				$resultadoTorneo->setPuntosPuesto($this);
			}
	 ...
	 * Otherwise if this PuntosPuesto is new, it will return
	 * an empty collection; or if this PuntosPuesto has previously
	 * been saved, it will retrieve related ResultadoTorneos from storage.
	 *
	 * This method is protected by default in order to keep the public
	 * api reasonable.  You can provide public methods for those you
	 * actually need in PuntosPuesto.
	 *
	 * @param      Criteria $criteria optional Criteria object to narrow the query
	 * @param      PropelPDO $con optional connection object
	...
	{
		return (string) $this->exportTo(PuntosPuestoPeer::DEFAULT_STRING_FORMAT);
	}
	...
	public function __call($name, $params)
	{
		...
		// symfony_behaviors behavior
		if ($callable = sfMixer::getCallable('BasePuntosPuesto:' . $name))
		{
