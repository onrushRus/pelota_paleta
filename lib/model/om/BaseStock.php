...
abstract class BaseStock extends BaseObject  implements Persistent
{
	...
	/**
	 * Get the [cantidad_actual] column value.
	 * 
	 * @return     int
	 */
	...
	public function setProductoId($v)
	{
		...
		if ($this->producto_id !== $v) {
			$this->producto_id = $v;
			$this->modifiedColumns[] = StockPeer::PRODUCTO_ID;
		}
	...
	/**
	 * Set the value of [cantidad_actual] column.
	 * 
	 * @param      int $v new value
	 * @return     Stock The current object (for fluent API support)
	 */
	public function setCantidadActual($v)
	{
		if ($v !== null) {
			$v = (int) $v;
		}

		if ($this->cantidad_actual !== $v) {
			$this->cantidad_actual = $v;
			$this->modifiedColumns[] = StockPeer::CANTIDAD_ACTUAL;
		}
	...
	public function setCantidadMinima($v)
	{
		...
		if ($this->cantidad_minima !== $v) {
			...
			$this->modifiedColumns[] = StockPeer::CANTIDAD_MINIMA;
		}
	...
	public function delete(PropelPDO $con = null)
	{
		...
		try {
			...
			foreach (sfMixer::getCallables('BaseStock:delete:pre') as $callable)
			{
			...
			if ($ret) {
				...
				$this->postDelete($con);
				// symfony_behaviors behavior
				foreach (sfMixer::getCallables('BaseStock:delete:post') as $callable)
				{
	...
	public function save(PropelPDO $con = null)
	{
		...
		try {
			...
			// symfony_behaviors behavior
			foreach (sfMixer::getCallables('BaseStock:save:pre') as $callable)
			{
			...
			if ($ret) {
				...
				$this->postSave($con);
				// symfony_behaviors behavior
				foreach (sfMixer::getCallables('BaseStock:save:post') as $callable)
				{
	...
	protected function doInsert(PropelPDO $con)
	{
		...
		if ($this->isColumnModified(StockPeer::CANTIDAD_ACTUAL)) {
			$modifiedColumns[':p' . $index++]  = '`CANTIDAD_ACTUAL`';
		}
	...
	public function __call($name, $params)
	{
		...
		// symfony_behaviors behavior
		if ($callable = sfMixer::getCallable('BaseStock:' . $name))
		{
