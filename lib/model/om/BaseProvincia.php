...
abstract class BaseProvincia extends BaseObject  implements Persistent
{
	...
	/**
	 * The value for the nombre_prov field.
	 * @var        string
	 */
	protected $nombre_prov;
	...
	/**
	 * Get the [nombre_prov] column value.
	 * 
	 * @return     string
	 */
	...
	/**
	 * Set the value of [nombre_prov] column.
	 * 
	 * @param      string $v new value
	 * @return     Provincia The current object (for fluent API support)
	 */
	public function setNombreProv($v)
	{
		if ($v !== null) {
			$v = (string) $v;
		}

		if ($this->nombre_prov !== $v) {
			$this->nombre_prov = $v;
			$this->modifiedColumns[] = ProvinciaPeer::NOMBRE_PROV;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->nombre_prov = ($row[$startcol + 1] !== null) ? (string) $row[$startcol + 1] : null;
			$this->resetModified();
	...
	protected function doInsert(PropelPDO $con)
	{
		...
		if ($this->isColumnModified(ProvinciaPeer::ID)) {
			$modifiedColumns[':p' . $index++]  = '`ID`';
		}
		if ($this->isColumnModified(ProvinciaPeer::NOMBRE_PROV)) {
			$modifiedColumns[':p' . $index++]  = '`NOMBRE_PROV`';
		}
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[1], $arr)) $this->setNombreProv($arr[$keys[1]]);
	}
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(ProvinciaPeer::NOMBRE_PROV)) $criteria->add(ProvinciaPeer::NOMBRE_PROV, $this->nombre_prov);
	...
	{
		$copyObj->setNombreProv($this->getNombreProv());
	 ...
	 * @return     PropelCollection|array Localidad[] List of Localidad objects
	 * @throws     PropelException
	 */
